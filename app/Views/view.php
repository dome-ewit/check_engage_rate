...
?>
          ...
          <td><?php echo $i ?></td>
          <td><a href= <?= @$value["link_post"]?> target='_blank' ><?php echo @$value['id_post'] ?> </td>
...
<tr align='center' class="bg-primary p-2 text-dark bg-opacity-10">
  ...
  <td><?= @$data["sum"]?></td>
  <td><?php echo "(", $data["sum"], "&nbsp;&nbsp;/&nbsp;&nbsp;", count($data["total_sum"]), ")", " = ", number_format($data["engage_average_63"], 2) ?></td>
  <td><?= @$data["follower_63"]?></td>
  <td><?php echo "(", number_format($data["engage_average_63"],2), "&nbsp;&nbsp;/&nbsp;&nbsp;", number_format($data["follower_63"]), ")", " * ", "100", " = ", @number_format($data["engage_rate_63"], 2) ?></td>
...
<table align='center' class="table table-bordered table-hover">
            <thead align='center' class="bg-primary p-2 text-dark bg-opacity-25">
              <th>post_total </th>
              <th>Engage_Rate different value </th>
            </thead>
            <tr align='center' class="bg-primary p-2 text-dark bg-opacity-10">
              <td>
                <p align='center'>
...
foreach ( $database_63 as $key => $value24) {?>
    <tr align='center' class="table-light">
      <td><?php echo $i ?></td>
      <td><?php echo $value24['influencer_id'] ?></td> 
      <td><?php echo $value24['network'] ?></td> 
      <td><?php echo number_format($value24['follower']) ?></td> 
      <td><?php echo number_format($value24['post_total']) ?></td> 
      <td><?php echo $value24['create_date'] ?></td> 
    </tr>
    <?php
    $i++;
  }
